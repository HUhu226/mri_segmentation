...
# 加载教师模型的参数
teacher_model = U_Net()
teacher_model.load_state_dict(torch.load('/tmp/code/brain_seg/p00/check/pth/0_4444_0_0.0001_10_0418_18:22:39.pth'))
...
for epoch in range(opt.max_epoch):
    ...
    for ii,(data,label) in tqdm(enumerate(train_dataloader),total=len(train_data), desc=f'Epoch{epoch}'):    #嵌套循环
        ...
        if ii%200==1:
            ...
            prefix = '/tmp/code/brain_seg/p00/check/pth/kd/' + str(acc)+'_4444_'+str(val_loss) + '_'+str(lr)+'_'+str(batch_size)+'_'
            ...
            name1 = time.strftime('/tmp/code/brain_seg/p00/check/plt/kd/' + '%m%d_%H:%M:%S.npy')   
